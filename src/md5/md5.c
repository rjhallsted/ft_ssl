...
/*   Updated: 2018/11/04 20:55:00 by rhallste         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_ftssl_md5_args		*get_args(int argc, char **argv)
{
	...
	while (i < argc)
	{
		if (!is_flag(argv[i]) && (fd = open(argv[i], O_RDONLY)) > 0) {
			args->input_fds = ft_memrealloc(sizeof(int) * (args->input_fd_count + 1));
			args->input_fds[args->input_fd_count] = fd;
			args->input_fd_count++;
		} else
			ft_printf_fd(STERR_FILENO, "%s\n", "md5: it: No such file or directory");
		i++;
	}
	if (args->input_fd_count == 0) {
		args->input_fd = ft_memalloc(sizeof(int));
