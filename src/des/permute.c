...
extern const unsigned int g_sbox[8][64];
...
extern const int g_key_shift[16];
extern const unsigned int g_comp_perm[48];
...
unsigned long			ftssl_des_key_transform(unsigned long *key, int round)
{
	...
	left = circle_shift_left(left, g_key_shift[round]);
	right = circle_shift_left(right, g_key_shift[round]);
	*key = left << 28;
	*key |= right;
	return (ftssl_des_permute(*key, 56, (unsigned int *)g_comp_perm, 48));
}

extern const unsigned int g_key_perm[56];
...
unsigned long			*ftssl_des_genkeys(unsigned long key, int reverse)
{
	...
	key = ftssl_des_permute(key, 64, (unsigned int *)g_key_perm, 56);
